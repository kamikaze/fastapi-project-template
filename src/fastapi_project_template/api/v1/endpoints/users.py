...
from fastapi_project_template.dto.users import UserProfile
...
@router.get('')
async def get_user_list(db_session: AsyncSessionDep, token: TokenDep) -> Page[UserProfile]:
    _ = await users_service.get_or_create_user_profile_by_token(db_session, token)
